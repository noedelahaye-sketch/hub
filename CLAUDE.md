...
- **Cas particulier formation** : le site de révision Bac+3 existant (https://noedelahaye-sketch.github.io/Bac-3/) reste indépendant. Il sauvegarde son avancée dans un gist GitHub. Le hub LIT ce gist en lecture seule pour afficher la progression des révisions dans l'espace formation. Ne jamais écrire dans ce gist.
  - **Le gist est privé et son identifiant n'existe nulle part dans le code source.** Vérifié le 6 août 2026 dans `js/app.js` du repo Bac-3 : le site ne stocke aucun id en dur, il appelle `GET /gists` avec un token personnel et retient le premier gist contenant un fichier nommé `studi-suivi-sync.json` (`findOrCreateGist`). L'id est ensuite gardé dans le `localStorage` du navigateur de Noé, sous la clé `studi-sync-gist-id`. Le gist est créé avec `public: false`.
  - Caractéristiques utiles : description « Suivi Studi — synchronisation », fichier `studi-suivi-sync.json`, contenu = état de révision sérialisé avec un champ `_ts` (horodatage servant à arbitrer entre local et distant).
  - **Identifiant du gist : à fournir par Noé.** Il se lit dans son navigateur sur le site Bac-3 (console : `localStorage.getItem("studi-sync-gist-id")`) ou dans https://gist.github.com/ une fois connecté.
  - **Conséquence à trancher avant de coder l'espace formation** : un gist privé ne se lit pas sans token GitHub, et aucun token ne doit figurer dans le repo public du hub. Trois options : rendre le gist public (le contenu n'est qu'un état de révision, sans donnée sensible) et le lire en anonyme ; passer la lecture par une Edge Function Supabase qui garde le token côté serveur ; ou faire écrire au site Bac-3 sa progression directement dans une table Supabase du hub. Tant que ce point n'est pas tranché, l'espace formation ne peut pas afficher la progression des révisions.
