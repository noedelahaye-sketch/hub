...
**LE SITE DU FC HERMITAGE FAIT DE MÊME depuis le 16 septembre 2026** (demande de
Noé), aux adresses `#hermitage/cap`, `#hermitage/objectif/<id>`,
`#hermitage/projet/<id>` et `#hermitage/taches`. Rien de nouveau n'a été écrit :
`versLaGalerieDuCap`, `versLObjectif` et `versLeProjet` déduisent leur base du
PREMIER SEGMENT de l'adresse courante, et `dansUnSite` remplace `dansLeSiteYuno`
dans les quatre modules — **une seule fonction, pour que le troisième site n'ait
rien à réapprendre**. *`tools/essai-cap-adresses.mjs` éprouve les trois bases hors
écran, `#hermitagexyz` compris.*
...
- `#hermitage` — le SITE FC Hermitage : l'habillage du hub disparaît, chrome et identité propres, fond bleu du club (voir docs/fch-spec.md). **IL MONTE LES ÉCRANS DU CAP depuis le 16 septembre 2026** — la galerie, la page d'un objectif, celle d'un projet, les tâches — aux adresses `#hermitage/cap`, `#hermitage/objectif/<id>`, `#hermitage/projet/<id>` et `#hermitage/taches` (demande de Noé : *« comme c'est fait sur Yuno »*). **Ce sont les modules du hub**, pas des copies, exactement comme chez Yuno : `js/cap-adresses.js` leur dit où mènent leurs liens selon l'écran où ils se dessinent, et deux gardes les empêchent de reprendre l'habillage du hub quand ils vivent dans le site. **Son accueil est un CLASSEMENT depuis le 16 septembre 2026** — une carte chaude tirée d'une cascade à cinq rangs, « À faire » en bloc cochable de trois lignes, trois portes tirées d'une réserve, le cap en pied. *La page ne change pas de forme, elle change de contenu : c'est la règle de l'accueil de Yuno, et elle répare le même défaut — 2 018 px de blocs fixes dont deux étaient du BILAN sur un écran qui est l'ATELIER, pendant que 65 engagements partenaires et une chaîne éditoriale bouchée à son premier cran n'apparaissaient nulle part.*
...
- `reproductible` boolean NOT NULL default true · `idee_mere_id` uuid REFERENCES publications(id) ON DELETE SET NULL — **une idée est un FORMAT, pas un contenu à faire une fois** (16 septembre 2026, règle de Noé : « le avant/après est reproductible plusieurs fois, donc une fois qu'il a été fait il ne doit pas disparaître ; cependant il peut y avoir des publications moins répétables, mon histoire, mon matériel »).
  - *Ce que ça répare* : programmer une idée posait sa date SUR SA PROPRE LIGNE — l'idée devenait la parution, quittait la banque, et une fois publiée n'y revenait jamais.
  - **Programmer un format fabrique une PARUTION** — une publication datée, ordinaire, reliée par `idee_mere_id` — et le format reste en banque. C'est la règle des séries : *les occurrences sont de VRAIES lignes*. Pas de table à part : le calendrier éditorial, le bilan du dimanche et `#temps` lisent tous `date_prevue`, et **une parution EST une publication**.
  - **Reproductible par défaut** : celle qui ne se refait pas est l'exception qu'on déclare, et **seule elle porte un mot** à l'écran. La parution, elle, naît en `idee` et porte `reproductible = false` — sans quoi la déprogrammer ferait un second format jumeau.
  - `ON DELETE SET NULL` : jeter un format ne jette pas ce qui en est sorti. Une parution peut être partie, porter son lien, compter dans un bilan.
  - **Chez Yuno seulement pour l'instant** : le site du FCH partage les gabarits mais pas le geste. Voir docs/yuno-spec.md.
...
- **`node --check` NE VÉRIFIE PAS UN MODULE ES** (16 septembre 2026). Il lit le
  fichier comme un script, et une liste d'import fautive — `{ CHEVRON,,
  friseDeLaSemaine }`, une virgule en trop — **passe sans un mot**. C'est le même
  piège que `tools/verifier-gabarits.js` comble pour les accents graves : le
  fichier est valide, seul le CHARGEMENT tombe, et il tombe en emportant tout
  l'écran. La vérification juste est `node --input-type=module --check < fichier`,
  qui refuse la même ligne en trois secondes.
...
- **Un HALL posé dans une `.bloc` doit égaler la spécificité de `.bloc ul`**
  (16 septembre 2026, **dixième fois** que ce piège se paie). `.bloc
  ul:not(.liste-jalons)` passe toute liste en grille de 21 rem au-delà de 60 rem
  et pèse **0-2-1** : `.fch-hall-lots` (0-1-0) perdait, la piste valait la largeur
  de son contenu, et l'ellipse posée sur le titre ne s'appliquait jamais.
  *Mesuré sur l'accueil du club : 315 px de contenu dans une colonne de 250, le
  compte poussé hors de la tuile.* Les deux autres halls du site y échappaient en
  vivant HORS des blocs — c'est la parade quand la section n'a pas de titre ;
  sinon on écrit `.bloc ul.<classe>`.
- **Avant de nommer une classe CSS OU UNE FONCTION, vérifier que le nom est libre — ET DANS LES TROIS FEUILLES.** *Payé une onzième fois le 16 septembre 2026 : `.evenement-grille`, choisie pour le calendrier d'un évènement du club, EST la fiche d'un moment du site Yuno (css/yuno.css). La section en héritait `grid-template-columns: auto minmax(0, 1fr)` — mesuré, la barre de période écrasée à 145 px dans une section de 743, et son contenu débordant de la page de 107 px.* *Payé une huitième fois le 15 septembre 2026 au soir : `.porte-semaine`, choisie pour la frise d'une porte du site Yuno, EST le bouton doré « Programmer ma semaine » de l'accueil du hub (css/styles.css, chargée sur les trois pages). La frise en héritait un aplat d'accent, une hauteur de contrôle et un `align-items: center` — un bandeau doré plein, les sept cases écrasées à 6 px et les initiales par-dessus.* `index.html` charge `css/styles.css`, `css/yuno.css` ET `css/fch.css` : un nom pris par la feuille d'un site s'applique au hub. Le 2 septembre 2026, `.projet-tete` (déjà pris par la tuile de projet du tableau de bord) a mis la tête de la page d'un projet sur une seule ligne, titre compris ; `.fiche-tete`, essayé ensuite, était pris par `css/fch.css`. Deux collisions pour une même tête. Le 1er septembre 2026, `construireHabitudesDuJour` a été écrite une seconde fois dans js/perso.js : le module s'est chargé sur « Identifier has already been declared » et a emporté tout l'écran. `node --check` passe — c'est un fichier valide —, seul le chargement tombe. La leçon est la même que pour `.barre` : un `grep` de trois secondes. `.barre`
...
- **Le glissement « d'une colonne vers un jour » vit dans `brancherPriseEnMain` (js/calendrier-commun.js)** depuis le 16 septembre 2026. Il était écrit TROIS FOIS, mot pour mot — « Ma semaine », la page d'un projet, celle d'un objectif —, et la fiche d'un évènement du club en aurait fait un quatrième. *Les trois d'origine tournent encore sur leur copie : les faire basculer est un chantier à part, et c'est exactement la divergence qu'on passe ensuite à rattraper — un jour l'une des quatre cessera d'avaler le clic qui suit un glissement, et ça ne se verra que sur l'écran qu'on regarde le moins.*
